...
function QuickPanel({ detail,functional,loading,error,tab,onTab,onRetry,onOpen }: { detail:DetailData|null; functional:FunctionalDetail|null; loading:boolean; error:string|null; tab:Tab; onTab:(tab:Tab)=>void; onRetry:()=>void; onOpen:(id:string)=>void }) {
  ...
  return <aside className="fintech-detail-panel">
    ...
    <div className="fintech-detail-grid"><Field label="Región" value={e.region}/><Field label="Comuna" value={e.commune}/><Field label="Vertical principal" value={e.primary_vertical}/><Field label="Modelo de negocio" value={e.business_model}/><Field label="Actividad SII" value={e.sii_main_activity}/><Field label="Tramo ventas" value={e.sii_sales_band ? `Tramo ${e.sii_sales_band}` : null}/><Field label="Condición CMF" value={e.has_cmf_public?'Huella pública':'Sin huella observada'}/><Field label="Condición UAF" value={e.has_uaf_public?'Inscrito público':'Sin registro público observado'}/><Field label="PSAV" value={psavLabel(functional?.psav_status ?? e.psav_status)}/><Field label="Estado operativo" value={operatingLabel(e.operating_status)} status={e.operating_status}/></div>
