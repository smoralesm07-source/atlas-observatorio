...
const METRIC_ORDER = [
  ...
  'COUNTRIES_SERVICE_REACH','COUNTRIES_OPERATING','PAYMENT_METHODS_COUNT','API_CONNECTIONS','AGREEMENTS_COUNT','HEALTH_PROVIDERS_NETWORK',
];
...
function formatMetricValue(row:LeaderRow) {
  ...
  const suffix:Record<string,string>={ users:' usuarios',clients:' clientes',companies:' empresas',merchants:' comercios',insured_persons:' asegurados',transactions:' tx',countries:' países',payment_methods:' medios',connections:' conexiones',agreements:' acuerdos',providers:' prestadores',policies:' pólizas',workers:' trabajadores' };
...
function shortMetric(code:string,label:string) {
  const map:Record<string,string>={
    ...
    TRANSACTIONS_MONTHLY_COUNT:'Transacciones / mes',TRANSACTIONS_QUARTERLY_COUNT:'Transacciones / trimestre',TRANSACTIONS_ANNUAL_COUNT:'Transacciones / año',
