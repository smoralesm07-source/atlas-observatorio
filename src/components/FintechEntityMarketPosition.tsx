...
function formatValue(row:Metric) {
  ...
  const suffix:Record<string,string>={rank:' tramo',workers:' trab.',users:' usuarios',clients:' clientes',companies:' empresas',merchants:' comercios',insured_persons:' asegurados',downloads:' descargas',transactions:' tx',orders:' órdenes',claims:' siniestros',verifications:' verificaciones',financings:' financiamientos',projects:' proyectos',institutions:' entidades',sources:' fuentes',countries:' países',payment_methods:' medios',connections:' API',agreements:' acuerdos',providers:' prestadores',policies:' pólizas',vehicles:' vehículos',implementations:' implementaciones'};
  return `${compact(row.value_numeric)}${suffix[row.unit ?? ''] ?? (row.unit ? ` ${row.unit}` : '')}`;
}
function shortLabel(code:string,label:string) {
  const map:Record<string,string>={SII_SALES_BAND_RANK:'Ventas SII',WORKERS:'Trabajadores',USERS:'Usuarios',CLIENTS:'Clientes',BUSINESS_CLIENTS:'Clientes empresa',MERCHANTS:'Comercios',INSURED_PERSONS:'Asegurados',POLICIES_ACTIVE_COUNT:'Pólizas vigentes',APP_DOWNLOADS_COUNT:'Descargas app',TRANSACTIONS_DAILY_COUNT:'Tx / día',TRANSACTIONS_MONTHLY_COUNT:'Tx / mes',ORDERS_MONTHLY_COUNT:'Órdenes / mes',TRANSACTIONS_QUARTERLY_COUNT:'Tx / trimestre',TRANSACTIONS_ANNUAL_COUNT:'Tx / año',CLAIMS_PROCESSED_COUNT:'Siniestros procesados',VERIFICATIONS_24H_COUNT:'Verificaciones / 24h',FINANCINGS_COUNT:'Financiamientos',PROJECTS_FINANCED_COUNT:'Proyectos financiados',DIRECT_PREMIUM_USD:'Prima directa',PROCESSED_VOLUME_MONTHLY_USD:'Volumen / mes',PROCESSED_VOLUME_PERIOD_USD:'Volumen del período',ANNUAL_TRANSACTION_VOLUME_USD:'Volumen anual',ANNUALIZED_TRANSACTION_VOLUME_USD_EST:'Volumen anualizado',CUMULATIVE_TRANSACTION_VOLUME_USD:'Volumen acumulado',TPV_USD:'TPV',ORIGINATED_VOLUME_USD:'Originación',AUM_AUC_USD:'AUM / AUC',INSURANCE_SALES_ENABLED_USD:'Ventas seguros habilitadas',FINANCIAL_INSTITUTIONS_AVAILABLE_COUNT:'Entidades disponibles',FINANCIAL_INSTITUTIONS_CONNECTED_COUNT:'Instituciones conectadas',DATA_SOURCES_CONNECTED_COUNT:'Fuentes conectadas',COUNTRIES_SERVICE_REACH:'Alcance países',COUNTRIES_OPERATING:'Países operando',PAYMENT_METHODS_COUNT:'Métodos de pago',API_CONNECTIONS:'Conexiones API',CONNECTED_VEHICLES_COUNT:'Vehículos conectados',IMPLEMENTATIONS_COUNT:'Implementaciones',AGREEMENTS_COUNT:'Acuerdos',HEALTH_PROVIDERS_NETWORK:'Prestadores'};
