...
select m.*,
       ...
       case
         ...
         when sc.scope_status='OUT_OF_SCOPE_CURRENT' then 'OUT_OF_SCOPE_CURRENT'
         when sc.scope_status='HISTORICAL_FINTECH' then 'HISTORICAL_ONLY'
...
alter table public.aml_fintech_ecosystem_coverage_snapshot
  add column if not exists semantic_resolved_unique integer,
  add column if not exists semantic_open_candidates_unique integer,
  add column if not exists semantic_product_resolved_unique integer,
  add column if not exists semantic_out_of_scope_unique integer,
  add column if not exists scope_assessed_unique integer,
  add column if not exists scope_included_unique integer,
  add column if not exists scope_excluded_unique integer;
...
begin
  ...
  select coalesce(v_ref.source_code,'UNKNOWN'),v_ref.source_date,v_ref.reported_total,
    ...
    (select count(*) from public.aml_v_fintech_ecosystem_semantic_current where semantic_stage in ('ENTITY_RESOLVED','NON_ENTITY_PRODUCT_RESOLVED','OUT_OF_SCOPE_CURRENT','HISTORICAL_ONLY')),
    ...
    (select count(*) from public.aml_v_fintech_ecosystem_semantic_current where semantic_stage in ('OUT_OF_SCOPE_CURRENT','HISTORICAL_ONLY')),
    ...
    jsonb_build_object(
      'sources',(select coalesce(jsonb_agg(x),'[]'::jsonb) from (
        ...
        where source_code in ('FINTECHILE_MEMBERS','LATAMFINTECH_CHILE')
