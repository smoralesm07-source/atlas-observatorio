...
begin
  ...
  payload := jsonb_set(
    ...
    coalesce(payload->'national','{}'::jsonb) || jsonb_build_object(
      'with_uaf_public', registry_total,
  ...
  select coalesce(jsonb_agg(
    case
      when src->>'source_code'='UAF_PUBLIC' then
        src || jsonb_build_object(
          'volume', registry_total,
          'role', 'Perímetro público UAF relevante para Fintech',
          'notes', format(
            '%s RUT únicos en actividades Fintec y Otras Entidades Facultadas para Recibir Moneda Extranjera; %s ya caracterizados como fintech y %s pendientes de caracterización.',
            registry_total, confirmed_target, pending_target
          )
        )
      else src
    end
  ...
  into patched_sources
  from jsonb_array_elements(coalesce(payload->'sources','[]'::jsonb)) with ordinality as s(src,ord);

  payload := jsonb_set(payload,'{sources}',patched_sources,true);
  ...
  payload := payload || jsonb_build_object(
    'uaf_perimeter', jsonb_build_object(
      ...
      'method_note','El KPI y la fila UAF de Fuentes representan el perímetro registral relevante para el estudio Fintech. La inscripción UAF por sí sola no convierte automáticamente a una entidad de Otras Entidades Facultadas para Recibir Moneda Extranjera en fintech confirmada.'
    )
  );
