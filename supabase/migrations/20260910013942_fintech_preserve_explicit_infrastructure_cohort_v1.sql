...
begin
  ...
  select e.fintech_id,
    case
      ...
      when e.primary_vertical in ('Infraestructura tecnológica','Open Finance','Regtech') then 'FINANCIAL_INFRASTRUCTURE'
      when exists (
        select 1 from public.aml_fintech_function_observation o join public.aml_fintech_function_catalog c using(function_code)
      ...
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code in ('FIN_PAYMENTS','FIN_REMITTANCE_FX','FIN_CARDS')) then 'PAYMENTS_REMITTANCES'
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code in ('FIN_LENDING','FIN_CROWDFUNDING')) then 'LENDING_FINANCING'
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code in ('FIN_INVESTMENT','FIN_INVESTMENT_ADVISORY','FIN_FINANCIAL_INTERMEDIATION','FIN_ALT_TRADING_SYSTEM','FIN_FINANCIAL_CUSTODY')) then 'WEALTH_MARKETS'
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code='FIN_INSURTECH') then 'INSURTECH'
    ...
    case
      ...
      when e.primary_vertical in ('Infraestructura tecnológica','Open Finance','Regtech') then 'Infraestructura financiera'
      when exists (
        select 1 from public.aml_fintech_function_observation o join public.aml_fintech_function_catalog c using(function_code)
        where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and (c.fatf_vasp or c.function_group='VA_CONTEXT')
      ) then 'Activos digitales'
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code in ('FIN_PAYMENTS','FIN_REMITTANCE_FX','FIN_CARDS')) then 'Pagos, remesas y banca digital'
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code in ('FIN_LENDING','FIN_CROWDFUNDING')) then 'Crédito y financiamiento'
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code in ('FIN_INVESTMENT','FIN_INVESTMENT_ADVISORY','FIN_FINANCIAL_INTERMEDIATION','FIN_ALT_TRADING_SYSTEM','FIN_FINANCIAL_CUSTODY')) then 'Wealth, inversión y mercados'
      when exists (select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id and o.evidence_status='OBSERVED' and o.function_code='FIN_INSURTECH') then 'Insurtech'
      else 'Infraestructura financiera'
    end,
    e.primary_vertical,
    case
      when e.primary_vertical in ('Activos Digitales','Activos Digitales / Pagos internacionales','Procesamiento de pagos y remesas','Paytech','Neobancos','Pagos / gestión financiera para Pymes','Mercados / FX institucional','Financiamiento alternativo y préstamos','Crédito Digital','Crowdfunding','WealthTech','WealthTech / Asesoría de inversión','Intermediación de Instrumentos Financieros','Sistemas Alternativos de Transacción','Custodia de Instrumentos Financieros','Insurtech','Gestión de finanzas personales y empresariales','BFM','PFM','Infraestructura tecnológica','Open Finance','Regtech') then 'NORMALIZED_VERTICAL'
    ...
    (select o.function_code from public.aml_fintech_function_observation o
      ...
      order by case o.function_code
        ...
        when 'FIN_OPEN_FINANCE' then 50 when 'FIN_REGTECH' then 51 when 'FIN_CREDIT_DECISIONING' then 52 when 'FIN_CARDS' then 60 else 99 end,
