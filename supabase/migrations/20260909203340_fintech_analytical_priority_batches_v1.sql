...
with lim as (
  ...
  select e.*,
    (select count(distinct es.source_code) from public.aml_fintech_entity_source es where es.fintech_id=e.fintech_id) as source_count,
    exists(select 1 from public.aml_fintech_regulatory_status r where r.fintech_id=e.fintech_id and r.regulator='UAF' and r.status='INSCRITO_PUBLICADO') as has_uaf,
    exists(select 1 from public.aml_fintech_regulatory_status r where r.fintech_id=e.fintech_id and r.regulator='CMF' and r.status in ('VIGENTE','AUTORIZADO','INSCRITO')) as has_cmf
  from public.aml_fintech_entity e
  where e.operating_status in ('ACTIVE','LIMITED','NO_NEW_BUSINESS')
  ...
  order by
    ...
    case when e.has_uaf then 0 else 1 end,
    case when e.has_cmf then 0 else 1 end,
    case when e.primary_vertical in ('Procesamiento de pagos y remesas','Paytech','Activos Digitales','Negociación de activos financieros y digitales','Financiamiento alternativo y préstamos','Crédito Digital','Infraestructura tecnológica') then 0 else 1 end,
    e.source_count desc,
...
with lim as (
  ...
  select e.*,
    ...
    exists(select 1 from public.aml_fintech_regulatory_status r where r.fintech_id=e.fintech_id and r.regulator='UAF' and r.status='INSCRITO_PUBLICADO') as has_uaf,
    exists(select 1 from public.aml_fintech_regulatory_status r where r.fintech_id=e.fintech_id and r.regulator='CMF' and r.status in ('VIGENTE','AUTORIZADO','INSCRITO')) as has_cmf,
    exists(select 1 from public.aml_fintech_function_observation o where o.subject_type='ENTITY' and o.subject_key=e.fintech_id) as has_function
  from public.aml_fintech_entity e
  where e.operating_status in ('ACTIVE','LIMITED','NO_NEW_BUSINESS')
    and e.website is not null and btrim(e.website)<>''
), ent as (
  ...
  from ent_ranked e
  order by
    case when not e.has_function then 0 else 1 end,
    case when e.has_uaf then 0 else 1 end,
    case when e.has_cmf then 0 else 1 end,
    case when e.primary_vertical in ('Procesamiento de pagos y remesas','Paytech','Activos Digitales','Negociación de activos financieros y digitales','Financiamiento alternativo y préstamos','Crédito Digital','Infraestructura tecnológica') then 0 else 1 end,
    e.source_count desc,
