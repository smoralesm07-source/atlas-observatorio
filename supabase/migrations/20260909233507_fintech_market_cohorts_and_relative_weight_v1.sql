create table if not exists atlas_private.aml_fintech_market_cohort_snapshot_v1 (
  fintech_id text primary key references public.aml_fintech_entity(fintech_id) on delete cascade,
  cohort_code text not null check (cohort_code in ('PAYMENTS_REMITTANCES','DIGITAL_ASSETS','LENDING_FINANCING','WEALTH_MARKETS','INSURTECH','FINANCE_MANAGEMENT','FINANCIAL_INFRASTRUCTURE')),
  cohort_label text not null,
  source_vertical text,
  assignment_basis text not null,
  assignment_confidence numeric not null check (assignment_confidence between 0 and 1),
  primary_function_code text,
  function_count integer not null default 0,
  metric_count integer not null default 0,
  refreshed_at timestamptz not null default now()
);

comment on table atlas_private.aml_fintech_market_cohort_snapshot_v1 is
'ATLAS Fintech analytical cohort layer. It normalizes legacy/current vertical labels for comparison while preserving aml_fintech_entity.primary_vertical as source classification. Cohorts are analytical, not regulatory categories.';

create index if not exists aml_fintech_market_cohort_v1_code_idx
  on atlas_private.aml_fintech_market_cohort_snapshot_v1(cohort_code, fintech_id);
...
begin
  ...
  select e.fintech_id,
    case
      ...
      when e.primary_vertical in ('Negociación de activos financieros y digitales','Activos Digitales / WealthTech')
           and exists (
             ...
             where o.subject_type='ENTITY' and o.subject_key=e.fintech_id
               ...
               and (c.fatf_vasp or c.function_group='VA_CONTEXT')
           ) then 'DIGITAL_ASSETS'
    ...
    e.primary_vertical,
    case when e.primary_vertical in ('Negociación de activos financieros y digitales','Activos Digitales / WealthTech') then 'NORMALIZED_VERTICAL_PLUS_OBSERVED_FUNCTIONS' else 'NORMALIZED_VERTICAL' end,
    case when e.primary_vertical is null then 0.80 else 0.99 end,
    (
      select o.function_code
      ...
      order by
        ...
        o.confidence desc,o.function_code
      limit 1
    ),
...
begin
  ...
  select jsonb_build_object(
    ...
    'methodology','Cohortes analíticas normalizan verticales equivalentes sin reemplazar la clasificación fuente. El peso se compara sólo dentro de la misma métrica y la misma cohorte, con al menos 3 pares. Usuarios, clientes, comercios, transacciones, volumen, alcance y métodos de pago nunca se suman entre sí y no constituyen cuota de mercado.'
  ) into outj;
  return outj;
end
$$;
...
begin
  ...
  select jsonb_build_object(
    ...
    'note','Percentil cohorte es la comparación preferente para entidades y sólo existe con al menos 3 pares en la misma métrica. Percentil global se conserva como referencia secundaria. No se suman métricas heterogéneas.'
  ) into outj;
  return outj;
end
$$;

revoke all on function atlas_private.aml_fintech_refresh_market_cohorts_v1() from public, anon, authenticated;
revoke all on table atlas_private.aml_fintech_market_cohort_snapshot_v1 from public, anon, authenticated;
revoke execute on function public.obs_fintech_market_weight_status() from public, anon;
revoke execute on function public.obs_fintech_market_weight_detail(text,text) from public, anon;
