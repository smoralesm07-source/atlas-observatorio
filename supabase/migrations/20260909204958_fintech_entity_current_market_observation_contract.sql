...
select
    ...
    (select count(*) from public.aml_fintech_market_metric_observation o where o.subject_type='ENTITY' and o.subject_key=f.fintech_id and o.evidence_type<>'NO_OBSERVABLE') as market_metric_count,
    (select max(o.observed_at) from public.aml_fintech_market_metric_observation o where o.subject_type='ENTITY' and o.subject_key=f.fintech_id and o.evidence_type<>'NO_OBSERVABLE') as market_metric_last_at,
    f.operating_status,
...
revoke all on public.aml_v_fintech_entity_current from public,anon;
grant select on public.aml_v_fintech_entity_current to authenticated,service_role;